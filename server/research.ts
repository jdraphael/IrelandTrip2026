...
export async function answerResearchQuestion({ question, deep = false, context, apiKey, db }: ResearchOptions): Promise<ResearchAnswer> {
  ...
  const prompt = [
    ...
    `Current itinerary JSON: ${JSON.stringify(itinerary.map((day) => ({ id: day.id, day: day.day, title: day.title, base: day.base, route: day.route, notes: day.notes, stops: day.stops.map((stop) => ({ id: stop.id, name: stop.name })) })))}.`,
